...
export function BoxScoreTable({ teamName, team, players, playerStats }: BoxScoreTableProps) {
  ...
  return (
    <div className={`bg-surface rounded-xl overflow-hidden border-t-4 ${accentBorder}`}>
      ...
      <table className="w-full">
        ...
        <tbody>
          {sorted.map(player => {
            const stats = playerStats[player.id] ?? { points: 0, fgMakes: 0, fgAttempts: 0, threeMakes: 0, threeAttempts: 0, ftMakes: 0, ftAttempts: 0 }
            return (
              <tr key={player.id} className="border-b border-[var(--color-border)] last:border-0">
                ...
                <td className="px-3 py-3 text-right font-body text-sm text-muted tabular-nums">{stats.fgMakes}/{stats.fgAttempts}</td>
                <td className="px-3 py-3 text-right font-body text-sm text-muted tabular-nums">{stats.threeMakes}/{stats.threeAttempts}</td>
                <td className="px-3 py-3 text-right font-body text-sm text-muted tabular-nums">{stats.ftMakes}/{stats.ftAttempts}</td>
              </tr>
            )
          })}
          {sorted.length === 0 && (
